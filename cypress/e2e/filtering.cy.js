...
describe("Filtering", () => {
    ...
    describe("Combined genre and title", () => {
        it("show movies with both title and genre", () => {
            const searchString = "m"; // filter by 'm' in the title
            const selectedGenreId = 18; // filter by genre
            const selectedGenreText = "Drama";

            //filter movies by title
            const moviesWithTitle = filterByTitle(movies, searchString);
            //filter by genre
            const moviesWithGenre = filterByGenre(moviesWithTitle, selectedGenreId);
            ...
            cy.get(".MuiCardHeader-content").should(
                "have.length",
                moviesWithGenre.length
            );
            cy.get(".MuiCardHeader-content").each(($card, index) => {
                cy.wrap($card).find("p").contains(moviesWithGenre[index].title);
            });
