...
describe("Navigation", () => {
    ...
    describe("From the favourites page to a movie's details", () => {
        ...
        beforeEach(() => {
            // Select one favourite and navigate to Favourites page
            cy.get("button[aria-label='add to favorites']").eq(3).click();
            cy.get("button").contains("Favorites").click();
        });

        it("navigates to the movie details page from favorites and change browser URL", () => {
            // Click the "More Info" button of the added movie
            cy.get(".MuiCardActions-root").eq(0).contains("More Info").click();
